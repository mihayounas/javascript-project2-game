    ...
    cardsArray.forEach((card) => {
        card.addEventListener('click', (event) => {
            ...
            if (cardElement.style.transform !== 'rotateY(180deg)' && cardElement.classList.contains('flip-card-inner')) {
                ...
                var audio = new Audio("../sounds/Card-flip-sound-effect.mp3");
                audio.play();
...
function matchOption(selectedOption) {
    ...
    if (option1 !== '' && option2 !== '') {
        if (option1 === option2) {
            ...
            var audio = new Audio("../sounds/Good-idea-bell.mp3");
            audio.play();
        } else {
            flipBack(option1);
            flipBack(option2);
            var audio = new Audio("../sounds/Error-sound.mp3");
            audio.play();
            option1 = '';
            option2 = '';
        }
...
function stopGame() {
    ...
    var audio = new Audio("../sounds/Winning-game-sound-effect.mp3");
    audio.play();
}